...
def wait_until(launcher: ProcessLauncher, running: RunningProcess, predicate, timeout: float = 5.0) -> None:
...
def test_basic_process_startup(launcher: ProcessLauncher):
    ...
    running = launcher.start(
        ProcessSpec(command=[sys.executable, "-c", "print('hello')"])
    )
    wait_until(launcher, running, lambda r: len(r.stdout_lines) > 0)
    assert "hello" in running.stdout_lines[0]
...
def test_standard_error_capture(launcher: ProcessLauncher):
    ...
    wait_until(launcher, running, lambda r: len(r.stderr_lines) > 0)
    assert "err" in running.stderr_lines


def test_normal_process_completion(launcher: ProcessLauncher):
    """3. Normal process completion."""
    running = launcher.start(
        ProcessSpec(command=[sys.executable, "-c", "print('done')"])
    )
    wait_until(launcher, running, lambda r: not launcher.is_running(running))
...
"""
    ...
    wait_until(launcher, running, lambda r: "alive" in r.stdout_lines)
    launcher.stop(running)
    assert not launcher.is_running(running)
...
def test_stop_all(launcher: ProcessLauncher):
    ...
    launcher.stop_all()
    wait_until(launcher, r1, lambda r: not launcher.is_running(r1))
    wait_until(launcher, r2, lambda r: not launcher.is_running(r2))
...
def test_bounded_output():
    ...
    wait_until(small, running, lambda r: len(r.stdout_lines) > 0)
    time.sleep(0.5)
    assert len(running.stdout_lines) <= 5
